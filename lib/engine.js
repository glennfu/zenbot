  ...
  function placeOrder (type, opts, cb) {
    ...
    if (so.mode !== 'live') {
      ...
      s.exchange[type](order_copy, function (err, api_order) {
        ...
        // FIXME: I don't know where the right place for this is now
        if (!s[type + '_order']) {
        ...
        // Pretty sure this is going to subscribe to this event multiple times, but I don't know where else to put this
        eventBus.on('ordersUpdated', function(err, api_order) {
          console.log(`ordersUpdated: ${JSON.stringify(api_order)}`)
